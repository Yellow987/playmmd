...
import { IMmdRuntime } from "babylon-mmd";
import { IAudioPlayer } from "babylon-mmd/esm/Runtime/Audio/IAudioPlayer";
...
export class MmdPlayerControl {
  ...
  public constructor(
    ...
    const parentControl = (this._newCanvasContainer =
      this._createCanvasContainer(rootElement.parentElement!));
    ...
    mmdRuntime.onAnimationDurationChangedObservable.add(
      this._onAnimationDurationChanged,
    );
  ...
  private _createPlayerControl(
    ...
    const playerContainer = (this._playerContainer =
    ...
    playerContainer.onmouseleave = this._onPlayerControlMouseLeave;
    {
      ...
      playerInnerContainer.style.background =
      ...
      playerContainer.appendChild(playerInnerContainer);
      {
        ...
        {
          const timeSlider = (this._timeSlider =
        ...
        playerInnerContainer.appendChild(playerLowerContainer);
        {
          ...
          playerLowerContainer.appendChild(playerLowerLeftContainer);
          {
            ...
            if (audioPlayer !== undefined) {
              const soundButton = (this._soundButton =
                ownerDocument.createElement("button"));
              ...
              const volumeSlider = (this._volumeSlider =
                ownerDocument.createElement("input"));
            ...
            const curentFrameNumber = (this._currentFrameNumberSpan =
            ...
            const endFrameNumber = (this._endFrameNumberSpan =
              ownerDocument.createElement("span"));
  ...
  private readonly _onAnimationTick = (): void => {
    ...
    if (this.displayTimeFormat === DisplayTimeFormat.Seconds) {
      ...
      this._currentFrameNumberSpan.innerText = Math.floor(
  ...
  private _getFormattedTime(time: number): string {
    // 00:00 or 0:00
  ...
  public dispose(): void {
    ...
    this._mmdRuntime.onPlayAnimationObservable.removeCallback(
      this._onAnimationPlay,
    );
    this._mmdRuntime.onPauseAnimationObservable.removeCallback(
      this._onAnimationPause,
    );
    this._mmdRuntime.onAnimationDurationChangedObservable.removeCallback(
      this._onAnimationDurationChanged,
    );
    this._mmdRuntime.onAnimationTickObservable.removeCallback(
      this._onAnimationTick,
    );
    this._audioPlayer?.onMuteStateChangedObservable.removeCallback(
      this._onMuteStateChanged,
    );
