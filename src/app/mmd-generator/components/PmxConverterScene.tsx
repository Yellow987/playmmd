...
class PmxConverterSceneBuilder implements ISceneBuilder {
  public async build(
    ...
    const loadModelAsync = async (file: File): Promise<void> => {
      ...
      mesh = await loadAssetContainerAsync(file, scene, {
        onProgress: (event) =>
          (engine.loadingUIText = `<br/><br/><br/>Loading (${file.name})... ${event.loaded}/${event.total} (${Math.floor((event.loaded * 100) / event.total)}%)`),
        ...
        OiComputeTransformInjector.OverrideComputeTransformMatrices(
          mmdMesh.metadata.skeleton!,
        );
    ...
    const switchMaterialTransparencyMode = (
      mode: "Alpha Mode" | "Force Depth Write Mode",
    ): void => {
      if (mesh === null) return;
      ...
      if (mode === "Alpha Mode") {
        ...
        for (let i = 0; i < materials.length; ++i) {
          const material = materials[i] as MmdStandardMaterial;
          material.transparencyMode = translucentMaterials[i]
    ...
    const renderLoadedFiles = (): void => {
      ...
      for (const file of files) {
        ...
        const fileRelativePath = file.webkitRelativePath as string;
        item.textContent = fileRelativePath.substring(
    ...
    const renderMaterialsList = (): void => {
      ...
      for (let i = 0; i < meshes.length; ++i) {
        ...
        if (currentMode === "Alpha Mode") {
          ...
          transparencyModeButton.textContent = fromTransparencyModeEnumToString(
          ...
          transparencyModeButton.onclick = (): void => {
            ...
            transparencyModeButton.textContent =
              fromTransparencyModeEnumToString(material.transparencyMode ?? 0);
          };
          ...
          opaqueToggleButton.textContent =
          ...
          opaqueToggleButton.onclick = (): void => {
            material.transparencyMode =
              material.transparencyMode === Material.MATERIAL_OPAQUE
                ? Material.MATERIAL_ALPHABLEND
                : Material.MATERIAL_OPAQUE;
            translucentMaterials[materialIndex] =
      ...
      function fromTransparencyModeEnumToString(
    ...
    preserveSerializationDataLabel.title =
      "If enabled, the converted file will be larger, but the converted file will be able to be converted back to PMX without any loss of data in technically(currently BPMX to PMX conversion is not supported).";
    ...
    preserveSerializationDataDiv.appendChild(
      preserveSerializationDataSmallLabel,
    );
    ...
    buildSkeletonLabel.title =
      "If your model don't need to be animated by skeleton(e.g. stage model), you can disable this option to reduce the size of the converted file. also, it can improve the performance of the converted model.";
    ...
    buildMorphLabel.title =
      "If your model don't need to be animated by morph targets(e.g. stage model), you can disable this option to reduce the size of the converted file. also, it can improve the performance of the converted model.";
...
function PmxConverterScene() {
  ...
  function setLocalModel(path: string) {
    dispath(
      setModels([
        {
          name: "local",
...
export default PmxConverterScene;
